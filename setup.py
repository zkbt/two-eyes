...
so that folks can `import cheerfulcamera` from within any directory.
...
...move a copy of this code to your site-packages directory, where
python will be able to find it (but you won't be able to keep editing
it), by typing *one* of the following commands:

`pip install .`
or
`python setup.py install`

...upload the entire package to the Python Package Index, so that
other folks will be able to install your package via `pip install`,
by running the following command:

`python setup.py release`

The template for this setup.py came was pieced together with help
from barentsen, christinahedges, timothydmorton, and dfm. Check them
out on github for more neat tricks!
...
if "release" in sys.argv[-1]:
    ...
    os.system("rm -rf dist/twoeyes*")
    sys.exit()

# a little kludge to get the version number from __version__
exec(open('twoeyes/version.py').read())
...
setup(
    ...
    description = "Stereogram images for ASTR1030.",
    # what's a more detailed description?
    long_description = "For detailed usage, please read the documentation at https://zkbt.github.io/two-eyes/.",
    ...
    url = "https://github.com/zkbt/two-eyes",
